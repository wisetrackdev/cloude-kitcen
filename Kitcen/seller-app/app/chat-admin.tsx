...
export default function SellerAdminChatScreen() {
  ...
  const sellerId = user?.id || 'seller-simulated';
  ...
  const fetchChats = async (showLoading = true) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/orders/support-seller-${sellerId}/chats`);
      if (res.ok) {
      ...
      console.warn('Failed to fetch support chats offline');
    } finally {
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: themeColors.border, backgroundColor: themeColors.card }]}>
        ...
        <View style={styles.headerInfo}>
          <Text style={[styles.title, { color: themeColors.text }]}>Admin Support</Text>
          <Text style={[styles.subtitle, { color: themeColors.textSecondary }]}>Shop Workspace Support</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={{ color: themeColors.textSecondary, marginTop: 10 }}>Loading support history...</Text>
          </View>
        ) : (
          <ScrollView 
            ref={scrollViewRef}
          ...
          >
            ...
            {messages.length === 0 && (
              <View style={styles.emptyContainer}>
                <Text style={{ color: themeColors.textSecondary, fontSize: 13, textAlign: 'center', lineHeight: 20 }}>
                  No messages yet. Ask Admin support team about your shop settings or payments!
                </Text>
              </View>
            )}
          </ScrollView>
